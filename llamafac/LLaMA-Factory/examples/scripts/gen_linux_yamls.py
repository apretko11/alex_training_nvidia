...
# Linux / “arm” column (Ubuntu-style) dataset bases (6 directions)
BASES = [
    "x86_to_armv8",
    "armv8_to_x86",
    "x86_to_riscv",
...
def yaml_dump_list(items, indent=2):
    return "\n".join((" " * indent + f"- {x}") for x in items)
...
def main():
    ...
    for base in BASES:
        ...
        if not shard_keys:
            raise SystemExit(
                f"No dataset_info keys found for base={base}. "
                f"(Expected keys like {base}_000, {base}_001, ...). "
                "Generate data + dataset_info.json first."
            )
